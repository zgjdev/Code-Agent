...
- 精确代码定位优先 `glob_files` / `grep_code` / `read_file`；`search_code` 只组合 SQLite FTS5 + BM25、BGE + cosine 和代码关系图三类召回，Symbol Index 仅供 Graph 内部生成 seed。内置 BGE 是可降级语义增强，远程 Embedding 必须有当前项目的显式授权。
