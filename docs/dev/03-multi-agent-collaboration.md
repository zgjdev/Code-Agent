...
> - 本文描述的是**代码实际做了什么**，包括「定义了但没人读」「写了但没人调用」「注释说的和实现不是一回事」「提示词有两个互相冲突的规则」这类真实落差；也记录**已经修掉的缺陷**（并行审查的数据竞争、失败重规划的无限递归）及其修法与回归测试。它不是通用工作流引擎教程，也不是「多 Agent 架构最佳实践」；持久化与 Task 边界恢复只按当前 `PlanStateStore` 的真实能力描述，不会把 tool-call 级续跑、共享黑板或分布式 Worker 写成已交付能力。
...
- **恢复边界是 Task，不是 tool call**：`Task.result/status` 与 DAG 结构现在由 `PlanStateStore` 持久化；child session 继续保存消息流水。进程退出后可跳过已完成节点并重试 `INTERRUPTED` 节点，但不会从某个已执行到一半的 tool call 精确续跑，也不会序列化 `TrustedUrlContext`。
